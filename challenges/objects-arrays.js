...
const universities = graduates.map(param => param.university)
universities.sort();
...
const contactInfo = graduates.map(param => `${param.first_name} ${param.email}`)
console.log(contactInfo);


/* Request 3: Find out how many universities have the string "Uni" included in their name. Create a new array called uni that contains them all. Log the result. */
const uniObjects = graduates.filter(param => param.university.includes(`Uni`))
const uni = uniObjects.map(elem => elem.university)
console.log(uni);
...
const lowerCase = zooAnimals.map(elem => elem.animal_name.toLowerCase())
...
*/
const populationTotal = zooAnimals.reduce((total, elem) => total + elem.population, 0);
